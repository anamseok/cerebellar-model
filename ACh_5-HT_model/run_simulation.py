...
'''
Based on CHaT model : https://github.com/trfore/chatmodel.git
implemented serotonin version
---------------------
...
def main():
    Conditions = ['Control','GABAzine','ACh','Sero']
...
def trial_sim(Condition):
    ...
    #add recurent inhibitory synapse
    GoC_eqs_sero = '''
    dv/dt   = (g_l*(E_l-v) + (g_e+g_n)*(E_e-v) + g_i*(E_i-v))/C_m : volt (unless refractory)    
    dg_e/dt = -g_e/tau_e : siemens
    dg_i/dt = -g_i/tau_i : siemens
    dg_n/dt = (-g_n + sigma_n_GoC * sqrt(tau_n) * xi)/tau_n : siemens
    g_e_tot_GoC : siemens
    ...
    if Condition in ('Control','GABAzine'):
        ...
        GoC = NeuronGroup(nGoC,
                          Equations(GoC_eqs_sero,
                                    g_l = g_l_GoC,
                                    E_l = -47*mV, #depolarization by serotonin
                                    E_e = E_e_GoC,
                                    E_i = E_i_GoC,
                                    C_m = C_m_GoC,
                                    tau_e = tau_e_decay_GoC
                                    tau_i = tau_i_decay_GoC),
                          threshold  = 'v > V_th_GoC',
                          reset      = 'v = V_r_GoC',
                          refractory = 'tau_r_GoC',
                          method     = 'euler')        
        ...
        GoC = NeuronGroup(nGoC,
                          Equations(GoC_eqs,
                                    g_l = g_l_GoC,
                                    E_l = -55*mV, #hyperpolarization by ACh
                                    E_e = E_e_GoC,
                                    E_i = E_i_GoC,
                                    C_m = C_m_GoC,
                                    tau_e = tau_e_decay_GoC),
    ...
    # Connect neurons #
    ###################
    if Condition in ('Control', 'GABAzine','Sero'):
    ...
    if Condition == 'GABAzine':
        GrC_GoC = Synapses(GoC,GrC,
                           on_pre = 'g_i += 0 * nS',
                           delay  = tau_r_GrC)
    elif Condition in ('Control','ACh','Sero'):
        GrC_GoC = Synapses(GoC,GrC,
    ...
    if Condition == 'Control':
        ...
        if random_weights:
            np.save('randomized_seroSpikeProb',spike_prob)
            np.save('seroConnections',connections)
            np.save('seroWeights', active_mf_GrC_weights)
            reduce_tonic = GrC.reduce_tonic[np.unique(GrC_M.j[active_indices,:])]
            np.save('tonicReduction',reduce_tonic)
            np.save('seroCondE',mean_cond_e)
            np.save('seroCondI',mean_cond_i)
        else:
            np.save('seroSpikeProb',spike_prob)
    else:
        print('ERROR: Unknown experimental condition')
